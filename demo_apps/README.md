...
To run Llama2 in Google Colab using [llama-cpp-python](https://github.com/abetlen/llama-cpp-python), follow the instructions in section "Running Llama2 Locally on Mac" to build quantized Llama2-13b-chat model `ggml-model-q4_0.gguf`, before uploading it to your Google drive. Note that on the free Colab T4 GPU, the call to Llama could take more than 20 minutes to return; running the notebook locally on M1 MBP takes about 20 seconds.
