...
def get_preprocessed_uniphore(dataset_config, tokenizer, split):
    os.system("df -h")
    raw_train_dataset = load_dataset("json", data_files=os.path.join(os.environ["SM_CHANNEL_TRAIN"],"train.jsonl"), cache_dir="opt/ml/input")['train']
    raw_validation_dataset = load_dataset("json", data_files=os.path.join(os.environ["SM_CHANNEL_TEST"],"val.jsonl"), cache_dir="opt/ml/input")['train']
